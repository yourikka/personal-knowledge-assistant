...
- 支持高频 Query 缓存和 SSE 流式问答输出，降低重复查询开销并改善交互体验
...
- `app/services/query_cache.py`：高频 Query 的 TTL/LRU 内存缓存
...
- Web 工作台支持快速入库和后台入库；快速入库会先落库并返回本地分类摘要，再通过后台任务补齐模型元数据、向量索引、图谱和关联。
- Web 工作台支持长期记忆管理；可以按会话查看当前会话和全局记忆，并编辑或删除过期、错误记忆。
...
说明：该接口优先快速返回，后台会自动提交单文档增强任务补齐模型元数据、向量索引、图谱和关联。
...
```bash
curl -X PATCH http://127.0.0.1:8010/api/memories/{memory_id} \
  -H "Content-Type: application/json" \
  -d '{
...
- 支持 Web 记忆管理，长期记忆可枚举、按会话筛选、编辑和删除
